(function () {
  ...
  function isGroupSystemEventMessage(value) {
    const text = cleanText(value);
    ...
    if (/\b(?:message|mensaje|photo|foto)\b/.test(normalized)) return false;
  ...
  async function processSnapshot({ automatic, detectedAtMs, settings, snapshot, snapshots, winnerIndex, followUpJob, followUpEligible, forceRefresh = false }) {
    ...
    if (isGroupSystemEventMessage(payload.currentMessage)) {
      lastCaptureHashByThread.set(threadKey, payload.messageHash);
      clearPendingBuyer(threadKey);
      await sendDebug("blocked", { ...debug, reason: "group_system_event" });
      return { skipped: true, reason: "group_system_event" };
    }
