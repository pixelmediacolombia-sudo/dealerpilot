...
const marketplaceKnowledgeMigrationSource = readFileSync(new URL("../../../../lib/db/migrations/0010_dealer_marketplace_knowledge.sql", import.meta.url), "utf8");
...
test("Sales AI intake is owned by the Messenger AI extension and backend contract", () => {
  ...
  assert.match(conversationsSource, /phone_received: "The buyer provided a phone number/);
  assert.match(conversationsSource, /closeAfterDelivery[\s\S]*phone_received/);
  assert.match(conversationsSource, /standaloneNumericAmount/);
  assert.match(conversationsSource, /buyerRequestsAnswerToPendingQuestion/);
  assert.match(conversationsSource, /buyerAskedLocation/);
  ...
  const cleanTitleStage = conversationsSource.indexOf('if (buyerAskedCleanTitle(latest)) return "clean_title";');
  const warrantyStage = conversationsSource.indexOf('if (buyerAskedWarrantyInfo(latest)) return "warranty_info";');
  const genericQuestionStage = conversationsSource.indexOf('if (buyerHasOpenQuestion(latest)) return "open_question";');
  assert.ok(combinedDetailStage >= 0 && combinedDetailStage < cleanTitleStage);
  ...
  assert.match(conversationsSource, /replyClaimsConfirmedAppointment/);
  assert.match(conversationsSource, /replyMentionsWrongVehicleYear/);
  assert.match(conversationsSource, /for \(let attempt = 0; attempt < 2; attempt \+= 1\)/);
  assert.match(conversationsSource, /Validation retry: the previous draft failed/);
  ...
  assert.match(conversationsSource, /dealer_phone=\$\{params\.storePhone\}/);
  assert.match(conversationsSource, /detailed_question/);
  assert.match(conversationsSource, /confirm that detail/);
  assert.match(conversationsSource, /Follow the supplied hasCleanTitleInventory configuration/);
  assert.match(conversationsSource, /hasCleanTitleInventory/);
  assert.match(dealerSchemaSource, /hasCleanTitleInventory: boolean\("has_clean_title_inventory"\)/);
  assert.match(dealerMigrationSource, /ADD COLUMN IF NOT EXISTS has_clean_title_inventory/);
  assert.match(dealersRouteSource, /hasCleanTitleInventory: z\.boolean\(\)\.optional\(\)/);
  assert.match(inventorySeedSource, /hasCleanTitleInventory: true/);
  assert.match(conversationsSource, /vin_inquiry/);
  assert.match(conversationsSource, /mileage_inquiry/);
  assert.match(conversationsSource, /color_inquiry/);
  assert.doesNotMatch(conversationsSource, /function addMarketplaceValueFact/);
  assert.match(conversationsSource, /Redacted copy brief \(authoritative/);
  assert.match(conversationsSource, /facts_to_deliver/);
  assert.match(conversationsSource, /Do not add mileage, price, color, VIN, or other feed facts/);
  assert.match(conversationsSource, /Estamos en \$\{knowledge\("address", storeAddress\)\}/);
  assert.match(conversationsSource, /We’re at \$\{knowledge\("address", storeAddress\)\}/);
  assert.match(conversationsSource, /provide the complete store address directly, give the dealership phone, and ask for the buyer's best phone number in the same reply/);
  ...
  assert.match(conversationsSource, /dealerKnowledge/);
  assert.match(conversationsSource, /test_drive_request/);
  assert.match(conversationsSource, /dealer_hours/);
  assert.match(conversationsSource, /trade_in_request/);
  assert.match(conversationsSource, /payment_methods_request/);
  assert.match(conversationsSource, /Mercedes-Benz GLB 250/);
  assert.match(conversationsSource, /not confirmed/);
  assert.match(dealerSchemaSource, /marketplaceKnowledge/);
  assert.match(marketplaceKnowledgeMigrationSource, /marketplace_knowledge/);
  assert.match(dealersRouteSource, /marketplaceKnowledge/);
