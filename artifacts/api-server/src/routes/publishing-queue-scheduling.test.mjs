...
test("Sales AI intake is owned by the Messenger AI extension and backend contract", () => {
  ...
  assert.match(conversationsSource, /Estamos en \$\{storeAddress\}\. ¿Te gustaría venir a verlo\? ¿Qué día te queda mejor\?/);
  assert.match(conversationsSource, /We’re at \$\{storeAddress\}\. Would you like to come see it\? What day works for you\?/);
