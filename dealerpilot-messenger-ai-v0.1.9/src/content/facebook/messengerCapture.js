(function () {
  ...
  const UI_TEXT = /^(?:marketplace|see details|ver detalles|more options|m[aá]s opciones|choose an emoji|choose a sticker|choose a gif|compose|chats|aa|unread message|write a message|escribe un mensaje|send a quick response|tap a response|enviar una respuesta|write to|escribe a|search|buscar|inbox|bandeja de entrada|selling|buying|comprando|vendiendo)$/i;
  ...
  function isLikelyBuyerNameCandidate(value) {
    ...
    if (/^(?:aa|choose an emoji|choose a sticker|choose a gif|compose|chats|unread message|write a message|escribe un mensaje)$/i.test(cleaned)) return false;
    if (/[?!:;]/.test(cleaned)) return false;
    if (isLikelyAutoReplyText(cleaned)) return false;
    const normalized = normalizeForMatch(cleaned);
    if (/\b(?:marketplace|facebook|listing|vehicle|toyota|honda|chevrolet|ford|tesla|rav4|camry|civic|equinox|tacoma|model y|available|interested|financing|message|sent|send|buyer|seller|response|emoji|sticker|gif|compose|chats|disponible|interesa|numero|vehiculo)\b/.test(normalized)) {
      return false;
    }
