(function () {
  ...
  function extractInboxPreviewMessage(document, buyerName) {
    ...
    for (const text of candidates) {
      ...
      const afterVehicle = text
        ...
        .trim();
      const fallbackMatch = text.match(new RegExp(`${buyer.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}[\\s\\S]{0,140}?(are you still interested\\?|what(?:'s| is) the best number[\\s\\S]{0,120}\\?|hola[\\s\\S]{0,120}\\?|is this available\\?)`, "i"));
  ...
  function captureFromRoot(root, sellerNameCandidates = [], documentRef = null) {
    ...
    const finalMessages = inboxPreview ? [...messages, inboxPreview] : messages;
    return {
      root,
      scope,
      buyerName,
