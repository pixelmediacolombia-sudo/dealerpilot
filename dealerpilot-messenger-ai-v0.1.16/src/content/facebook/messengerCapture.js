(function () {
  ...
  function isLikelyBuyerText(text) {
    ...
    if (/^(?:yes\b|no\b|s[ií]\b|no\b|ok\b|okay\b|thanks|gracias)/i.test(cleaned) && cleaned.length < 30) return false;
    return /\?/.test(cleaned) ||
      /\b(?:available|disponible|interested|interesa|still|todav[ií]a|price|precio|best number|mejor n[uú]mero|financing|financiamiento|hello|hola|hi\b|wanna|quiero|me gustar[ií]a|condition|condici[oó]n|miles|millas|vin|title|t[ií]tulo|history|historial|accident|accidente|clean title|negotiable|negociable|puedo|how much|costo|cost|offer|oferta)\b/i.test(cleaned);
  }
  ...
  function findThreadRoots({ document, location }) {
    ...
    const composerRoots = [];
    const composers = Array.from(document?.querySelectorAll?.(COMPOSER_SELECTORS.join(", ")) || []).filter(isComposer);
    const seenComposerSig = new Set();
    for (const composer of composers) {
      let parent = composer.parentElement;
      while (parent && parent !== document.documentElement) {
        const pr = rectOf(parent);
        const hasRoleOrLabel = parent.hasAttribute?.("role") || parent.hasAttribute?.("aria-label");
        if (isVisible(parent) && pr.left > 200 && pr.width > 300 && pr.height > 400 && hasRoleOrLabel) {
          const sig = `${Math.round(pr.left)}|${Math.round(pr.top)}|${Math.round(pr.width)}`;
          if (!seenComposerSig.has(sig)) {
            seenComposerSig.add(sig);
    ...
    return [...candidates, ...composerRoots]
      .filter((candidate) => {
        if (seenRef.has(candidate)) return false;
        seenRef.add(candidate);
        const sig = `${Math.round(rectOf(candidate).left)}|${Math.round(rectOf(candidate).top)}|${Math.round(rectOf(candidate).width)}`;
  ...
  function readVisualMessages(scope, buyerName) {
    ...
    for (const textElement of bubbleTextCandidates(scope)) {
      ...
      const contentOverride = clearlyRightAligned && isLikelyBuyerText(text) ? buyerName || "Buyer" : null;
      const speaker = contentOverride || (clearlyRightAligned ? "Dealer" : (buyerName || "Buyer"));
      rows.push({ top: rect.top, speaker, text });
    }
  ...
  function captureFromRoot(root, sellerNameCandidates = [], documentRef = null) {
    ...
    const semantic = Array.from(scope?.querySelectorAll?.(STRUCTURED_MESSAGE_SELECTORS.join(", ")) || [])
      ...
      .filter(Boolean);
    const visual = scope ? readVisualMessages(scope, buyerName) : [];
    const semanticMap = new Map();
    const merged = [];
    const seenText = new Set();
    for (const msg of semantic) {
      const key = normalizeForMatch(msg.text);
      if (!seenText.has(key)) {
        seenText.add(key);
        semanticMap.set(key, merged.length);
        merged.push(msg);
      }
    }
    for (const msg of visual) {
      const key = normalizeForMatch(msg.text);
      if (!seenText.has(key)) {
        seenText.add(key);
        merged.push(msg);
    ...
    const lastBuyerIndex = [...finalMessages].reverse().findIndex((m) => m.speaker !== "Dealer");
    const latestBuyerMessage = lastBuyerIndex >= 0 ? finalMessages[finalMessages.length - 1 - lastBuyerIndex] : null;
    return {
      root,
      scope,
      buyerName,
      ...
      evidence: {
        ...
        messageCandidateCount: Math.max(semantic.length, visual.length),
        inboxPreviewFallback: !!inboxPreview,
        extractionMode: semantic.length && !hasBuyerMessage && visual.length ? "hybrid" : semantic.length ? "semantic" : visual.length ? "visual_bubbles" : "none",
        latestMessageDirection: finalMessages.at(-1)?.speaker === "Dealer" ? "dealer" : finalMessages.length ? "buyer" : "none",
        latestBuyerMessage: latestBuyerMessage?.text || "",
      },
    };
  }
