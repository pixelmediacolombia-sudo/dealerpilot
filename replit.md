...
- Guardrail module: `artifacts/api-server/src/publishing/controlledMode.ts` — `resolvePublishMode(dealerAutoClickPublish)` (the saved Controlled Auto setting is authoritative; the server never trusts a client-sent `mode`), `isExtensionOnline()`, `LOT_CITY_MAP` (Manassas only), `checkPublishGuardrails()` (single source of truth for all pre-dispatch checks)
