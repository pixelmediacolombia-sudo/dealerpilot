(function () {
  ...
  function extractVisibleBuyerMessages(root, documentRef, buyerName) {
    const buyer = cleanDisplayName(buyerName);
    if (!buyer) return [];
    const sources = [
      ...Array.from(root?.querySelectorAll?.('div[dir="auto"], span[dir="auto"], [aria-label]') || []),
      ...Array.from(documentRef?.querySelectorAll?.('div[dir="auto"], span[dir="auto"], [aria-label]') || []),
    ].filter(isVisible);
    const seen = new Set();
    return sources
      .map((element) => ({
      ...
      .filter((candidate) => candidate.text && candidate.text.length >= 4 && !UI_TEXT.test(candidate.text))
  ...
  function extractBuyerName(root) {
    ...
    for (const candidate of candidates) {
      ...
      if (!/\b(?:marketplace|toyota|honda|chevrolet|ford|tesla|rav4|camry|civic|equinox|tacoma|model y|send|tap|response)\b/i.test(value)) {
        return value;
      }
    }
  ...
  function captureFromRoot(root, sellerNameCandidates = [], documentRef = null) {
    ...
    const inboxPreview = !hasBuyerMessage ? extractInboxPreviewMessage(documentRef, buyerName) : null;
    const visibleBuyerMessages = extractVisibleBuyerMessages(root, documentRef, buyerName);
