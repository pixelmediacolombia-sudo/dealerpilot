(function () {
  ...
  function isReliableBuyerName(name) {
    ...
    if (/^(?:unknown buyer|buyer|facebook)$/i.test(cleaned)) return false;
    if (/\b(?:marketplace|listing|vehicle|facebook|group|page)\b/i.test(cleaned)) return false;
    if (/[/$]/.test(cleaned)) return false;
    return true;
  }

  function detectListingContext(root = document) {
    ...
    return {
      ...
      vehicleTitle: cleanText(titleFromText || headingText).slice(0, 160),
    };
  }
  ...
  function createCaptureSnapshot(settings = DEFAULT_SETTINGS) {
    ...
    const context = detectListingContext(capture.root || document);
  ...
  function buildBuyersState(snapshots = []) {
  ...
  async function processSnapshot({ automatic, detectedAtMs, settings, snapshot, snapshots }) {
    const validation = validateSnapshot(snapshot);
    const debug = buildDebugBase({ automatic, settings, snapshot, validation, snapshots });
  ...
  async function captureConversationOnce(options = {}) {
    ...
    const actionable = snapshots.filter((snapshot) => snapshot.evidence.threadRootDetected);
    const targets = actionable.length ? actionable : snapshots;
    const results = [];
    for (const snapshot of targets) {
      results.push(await processSnapshot({ automatic, detectedAtMs, settings, snapshot, snapshots }));
    }
    if (results.length === 1) {
      return {
        ...results[0],
